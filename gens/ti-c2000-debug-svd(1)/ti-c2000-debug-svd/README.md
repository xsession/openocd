...
- Renode Monitor backend for `xsession/renode` `custom-cores`;
...
- protocol-faithful fake Renode monitor integration tests;
...
```sh
code --install-extension extension/c2000-debug-0.2.0.vsix
```
...
### Generate a Renode launch configuration

```powershell
.\.venv\Scripts\ti-svd.exe vscode-config tms320f28069 `
  --backend renode `
  --renode-path C:/tools/renode/renode.exe `
  --renode-script '${workspaceFolder}/examples/renode/f28069.resc' `
  --executable '${workspaceFolder}/build/application.out' `
  --output .vscode/f28069-renode.json
```

The Renode backend is matched to the `xsession/renode` `custom-cores` register and
address model. F28069 and F280049 platform descriptions are present in that branch;
F28M35x is not. See [docs/renode-custom-cores.md](docs/renode-custom-cores.md).
...
- Renode Monitor register, memory, stepping and hook behavior;
- static compatibility inspection of the custom C2000 branch;
- VSIX construction.
...
examples/renode/ Renode platform scripts
reports/         compatibility inspection snapshots
