...
- `docker-compose.yml` mounts `./config:/work/config:ro`.
...
```powershell
$env:OUT_DIR="docker/data/dist/windows"
sh scripts/build-windows-cross.sh
```
...
```sh
docker buildx build -f docker/Dockerfile.windows-cross --build-arg JOBS=4 --target export --output type=local,dest=docker/data/dist/windows .
```

Do not use `docker/Dockerfile.windows-cross` as the runtime service Dockerfile in `docker-compose.yml`. It is a package/export Dockerfile with a `scratch` export stage. Use:
