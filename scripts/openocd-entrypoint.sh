...
# Add /work as an OpenOCD script root. This makes Compose commands like
#   -f config/default.cfg
# work when ./config is mounted to /work/config.
# shellcheck disable=SC2086
exec openocd \
  -s "${OPENOCD_WORK_SCRIPT_DIR}" \
  -c "bindto ${OPENOCD_BINDTO}" \
  ${OPENOCD_EXTRA_ARGS} \
  "$@"
