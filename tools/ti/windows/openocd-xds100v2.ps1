...
if ($OpenOcdArgs -and $OpenOcdArgs.Count -gt 0) {
  ...
  $args += @('-f', 'board/ti/launchxl-f28069m-xds100v2.cfg')
}
...
$installer = Join-Path $PSScriptRoot 'install-xds100v2-winusb-mi00.ps1'
if (-not (Test-Path -LiteralPath $installer)) {
  Write-Warning "OpenOCD could not open the XDS100v2 debug interface, but the WinUSB installer is missing: $installer"
  exit $first.ExitCode
}

Write-Host ''
Write-Warning 'OpenOCD cannot open the XDS100v2 debug interface. The MI_00 debug port likely needs WinUSB.'
...
Write-Host ''
Write-Host 'Unplug and reconnect the LaunchPad USB cable, then press Enter to retry OpenOCD.'
