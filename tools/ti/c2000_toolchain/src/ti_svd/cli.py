...
def cmd_vscode_config(args: argparse.Namespace) -> int:
    ...
    if args.peripheral_viewer_only:
        ...
        if backend == "openocd":
            ...
            if manifest.id not in {"tms320f28069", "tms320f280049"}:
                raise RuntimeError(
                    f"the xsession Renode custom-cores branch has no tested platform for {manifest.id}"
                )
            default_script = (
                "f28069.resc" if manifest.id == "tms320f28069" else "f280049.resc"
            )
            config.pop("corePattern", None)
            config.pop("runToEntryPoint", None)
            config["renodePath"] = args.renode_path or "C:/tools/renode/renode.exe"
            config["renodeScript"] = args.renode_script or (
                f"${{workspaceFolder}}/examples/renode/{default_script}"
            )
            config["renodeHost"] = args.renode_host
            config["renodeMonitorPort"] = args.renode_monitor_port
            config["renodeCpu"] = args.renode_cpu
            config["renodeSysbus"] = args.renode_sysbus
            config["registerProfile"] = "renode-c2000"
            if args.renode_binary_load_address:
                config["renodeBinaryLoadAddress"] = args.renode_binary_load_address
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_vscode.add_argument("--backend", choices=["ccs", "renode", "openocd", "mock"], default="ccs")
    ...
    p_vscode.add_argument(
        "--register-profile",
        choices=["auto", "c28x", "c28x-fpu-vcu", "c28x-fpu-tmu-vcu", "cortex-m3", "renode-c2000"],
        default="auto",
    )
    p_vscode.add_argument("--openocd-host", default="127.0.0.1")
    p_vscode.add_argument("--openocd-telnet-port", type=int, default=4444)
    p_vscode.add_argument("--renode-path")
    p_vscode.add_argument("--renode-script")
    p_vscode.add_argument("--renode-host", default="127.0.0.1")
    p_vscode.add_argument("--renode-monitor-port", type=int, default=1234)
    p_vscode.add_argument("--renode-cpu", default="cpu")
    p_vscode.add_argument("--renode-sysbus", default="sysbus")
    p_vscode.add_argument("--renode-binary-load-address")
